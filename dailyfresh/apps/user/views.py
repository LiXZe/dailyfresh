...
def register(request):
    ...
    if request.method == 'GET':  # 通过url地址访问时是get方式请求
        # 显示注册页面
        ...
        if password != cpassword:
            return render(request, 'register.html', {'errmsg': '俩次密码不一致'})
        ...
        # 进行业务处理: 进行用户注册,调用django自带的认证系统User
...
def testCelery(request):
    ...
    return HttpResponse('testCelery!')
...
class LoginView(View):
    ...
    def post(self, request):
        ...
        # 业务处理:登录校验(使用django自带的用户认证系统获取数据库中的user)
        user = authenticate(username=username, password=password)
        # lxz = User.objects.get(username=username)
        # print(lxz.password)
        if user is not None:
            # 用户名密码正确
            if user.is_active:
                # 用户已激活
                # django自带的用户认证系统login来存储sessionID，记录用户的登录状态
                login(request, user)
...
class UserInfoView(LoginRequiredMixin, View):
    ...
    def get(self, request):
        ...
        con = get_redis_connection('default')  # 使用setting.py中默认的redis配置
